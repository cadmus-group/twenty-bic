...
With **Twenty’s** `packages/twenty-docker/docker-compose.dev.yml`, Postgres is published on **`localhost:5433`** by default (so it does not clash with another Postgres on `5432`). Set `PG_DATABASE_URL` in `packages/twenty-server/.env` to use that port (see `.env.example`).

<Warning>
If you still see `password authentication failed for user "postgres"`, your `PG_DATABASE_URL` host/port does not match the running container. Run `bash packages/twenty-utils/setup-dev-env.sh` to align `localhost` URLs with `TWENTY_DEV_PG_PORT`, or set `TWENTY_DEV_PG_PORT` and `PG_DATABASE_URL` to the same port explicitly.
</Warning>
